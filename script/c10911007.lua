...
function s.initial_effect(c)
	...
	ce1:SetHintTiming(TIMING_SPSUMMON)
	ce1:SetCondition(s.thcon2)
	...
	ce2:SetCode(EVENT_FREE_CHAIN)
	ce2:SetHintTiming(TIMING_SPSUMMON)
	ce2:SetCondition(s.thcon2)
...
function s.spop(e, tp, eg, ep, ev, re, r, rp)
	...
	if e:GetHandler():IsRelateToEffect(e) and Duel.SpecialSummon(e:GetHandler(), 0, tp, tp, false, false, POS_FACEUP) > 0 and Duel.IsPlayerCanDiscardDeck(tp, 1) then
		Duel.BreakEffect()
...
function s.filter1(c, tp, e)
	...
	c:IsCanBeFusionMaterial() and c:IsAbleToGrave() and (c:IsFaceup() or c:IsPublic()) and
