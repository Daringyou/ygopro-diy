...
function cm.filtercheck(c)
	return c:GetOriginalRace()&RACE_FAIRY~=0 and not c:IsLocation(LOCATION_DECK)
end
function cm.fcheck(tp,sg,fc)
	return sg:FilterCount(Card.IsLocation,nil,LOCATION_DECK)<=sg:FilterCount(cm.filtercheck,nil)
end
